...
Once that is cloned we need to add the pip dependencies. You can run the `craft` command:
...
This will install all the required dependencies to run this framework. As well as copy `.env-example` into `.env`. Now we can run the wsgi server by simply running `gunicorn -w 2 wsgi:application` or the `craft` command:

    $ craft serve

This will run the server at `127.0.0.1:8000`. Navigating to that URL should show the Masonite welcome message.
...
And navigate to `127.0.0.1:8000/hello/world` and you will see `Hello World` in your browser.
