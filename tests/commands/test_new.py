...
class TestNewCommand(unittest.TestCase):
    ...
    def test_handle_incorrect_branch(self):
        responses.add(
            responses.GET,
            "https://api.github.com/repos/{0}/branches/unknown".format(
                self.default_repo
            ),
            body="[]",
        )
    ...
    def test_handle_incorrect_version(self):
        ...
        self.assertEqual(
            "Version 0.0.0 could not be found.\n", self.command_tester.io.fetch_error()
        )
    ...
    def test_correct_version_is_displayed(self):
        ...
        responses.add_passthru(
            "https://api.github.com/repos/MasoniteFramework/cookie-cutter/zipball/v2.3.6"
        )
        responses.add_passthru(
            "https://codeload.github.com/MasoniteFramework/cookie-cutter/legacy.zip/v2.3.6"
        )

        self.command_tester.execute("new_project --release 2.3.6")
        self.assertTrue(
            self.command_tester.io.fetch_output().startswith(
                "Installing version v2.3.6"
            )
        )
    ...
    def test_warning_is_displayed_when_no_tags_in_repo_if_not_official(self):
        ...
        # authorize requests not using the API
        responses.add_passthru(
            "https://gitlab.com/api/v4/projects/samuelgirardin%2Fmasonite-tests/repository/archive.zip?sha=master"
        )
        self.command_tester.execute(
            "new_project --repo samuelgirardin/masonite-tests --provider gitlab"
        )
        self.assertTrue(
            "No tags has been found, using latest commit on master."
            in self.command_tester.io.fetch_output()
        )
    ...
    def test_api_rate_limit_are_handled(self):
        ...
        with patch(
            "six.moves.http_client.responses", get=lambda status: "rate limit exceeded"
        ):
    ...
    def test_unknown_repos_are_handled(self):
        ...
        with self.assertRaises(ProjectProviderHttpError) as e:
            self.command_tester.execute(
                "new_project --repo MasoniteFramework/unknown_repo"
            )
    ...
    def test_can_craft_default_repo_successfully(self):
        ...
        # authorize requests not using the API
        responses.add_passthru(
            "https://github.com/MasoniteFramework/cookie-cutter/archive/{0}.zip".format(
                self.default_branch
            )
        )
        responses.add_passthru(
            "https://codeload.github.com/MasoniteFramework/cookie-cutter/zip/{0}".format(
                self.default_branch
            )
        )
    ...
    def test_can_craft_default_repo_successfully_with_release(self):
        ...
        responses.add(
            responses.GET,
            "https://api.github.com/repos/{0}/releases/tags/v2.3.6".format(
                self.default_repo
            ),
        ...
        responses.add_passthru(
            "https://codeload.github.com/MasoniteFramework/cookie-cutter/legacy.zip/refs/tags/v2.3.6"
        )
    ...
    def test_can_craft_default_repo_successfully_with_branch(self):
        ...
        # authorize requests not using the API
        responses.add_passthru(
            "https://github.com/MasoniteFramework/cookie-cutter/archive/2.3.zip"
        )
        responses.add_passthru(
            "https://codeload.github.com/MasoniteFramework/cookie-cutter/zip/2.3"
        )
    ...
    def test_can_craft_project_with_gitlab_provider(self):
        ...
        self.command_tester.execute(
            "new_project --provider gitlab --repo {0}".format(repo)
        )
        self.assertTrue(
            "Project Created Successfully" in self.command_tester.io.fetch_output()
        )
