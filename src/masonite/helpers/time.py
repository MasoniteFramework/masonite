...
def cookie_expire_time(str_time):
    ...
    if str_time != 'expired':
        ...
        if length in ('second', 'seconds'):
            ...
            return pendulum.now('GMT').add(seconds=number).format('ddd, DD MMM YYYY H:mm:ss GMT')
        elif length in ('minute', 'minutes'):
            return pendulum.now('GMT').add(minutes=number).format('ddd, DD MMM YYYY H:mm:ss GMT')
        elif length in ('hour', 'hours'):
            return pendulum.now('GMT').add(hours=number).format('ddd, DD MMM YYYY H:mm:ss GMT')
        elif length in ('days', 'days'):
            return pendulum.now('GMT').add(days=number).format('ddd, DD MMM YYYY H:mm:ss GMT')
        elif length in ('week', 'weeks'):
            return pendulum.now('GMT').add(weeks=1).format('ddd, DD MMM YYYY H:mm:ss GMT')
        elif length in ('month', 'months'):
            return pendulum.now('GMT').add(months=number).format('ddd, DD MMM YYYY H:mm:ss GMT')
        elif length in ('year', 'years'):
            return pendulum.now('GMT').add(years=number).format('ddd, DD MMM YYYY H:mm:ss GMT')

        return None
    else:
        return pendulum.now('GMT').subtract(years=20).format('ddd, DD MMM YYYY H:mm:ss GMT')
