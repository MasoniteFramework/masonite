...
class Route:
    ...
    def get(self, url, controller, **options):
        route = HTTPRoute(url, controller, request_method=["get"], compilers=self.compilers, **options)
        self.routes.append(route)
        return route

    @classmethod
    def post(self, url, controller, **options):
        route = HTTPRoute(url, controller, request_method=["post"], compilers=self.compilers, **options)
        self.routes.append(route)
        return route

    @classmethod
    def put(self, url, controller, **options):
        self.routes.append(HTTPRoute(url, controller, request_method=["put"], compilers=self.compilers, **options))
        return self

    @classmethod
    def patch(self, url, controller, **options):
        route = HTTPRoute(url, controller, request_method=["patch"], compilers=self.compilers, **options)
        self.routes.append(route)
        return route

    @classmethod
    def delete(self, url, controller, **options):
        route = HTTPRoute(url, controller, request_method=["delete"], compilers=self.compilers, **options)
        self.routes.append(route)
        return route

    @classmethod
    def option(self, url, controller, **options):
        route = HTTPRoute(url, controller, request_method=["options"], compilers=self.compilers, **options)
        self.routes.append(route)
        return route
    ...
    def match(self, request_methods, url, controller, **options):
        route = HTTPRoute(url, controller, request_method=request_methods, compilers=self.compilers, **options)
        self.routes.append(route)
        return route
