Larapy has a clear distinction between HTTP routes and API routes. HTTP routes are housed inside `routes/web.py`. All routes should be inside the `routes=[]` list so that the gunicorn engine can grab it.
